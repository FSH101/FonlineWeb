...
const fallbackUiConfig = {
  ...
  Player: {
    RunAnimation: 'player/hero_run.fr',
    IdleAnimation: '',
...
function applyPlayerSettings(config) {
  ...
  playerSettings.idlePath = config.IdleAnimation?.trim() || '';
...
function resolveDirectionIndex(direction, animation) {
  ...
  if (direction) {
    const idx = playerSettings.directionOrder.indexOf(direction);
    if (idx !== -1) {
      return Math.min(idx, animation.directions.length - 1);
    }
  }

  const fallbackDirection = playerSettings.defaultFacing;
  const fallbackIndex = playerSettings.directionOrder.indexOf(fallbackDirection);
  if (fallbackIndex !== -1) {
