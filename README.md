...
> ℹ️ **Автоматическая загрузка архива.** Если локальных GIF нет, клиент скачает и распакует архив `fr_gifs_pack.zip` из [релизов репозитория](https://github.com/FSH101/FonlineWeb/releases/tag/fr-gifs-20251005-175324) прямо в браузере. Файлы извлекаются из вложенного архива `fr_gifs_pack.zip/gif/*.gif`, кешируются в памяти и переиспользуются при повторных запросах. Никакой дополнительной установки не требуется — достаточно выбрать персонажа в селекторе или указать его код в конфиге.
