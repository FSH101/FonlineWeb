...
В каталоге `client/` находится простая визуализация гексовой сетки с заглушками графики и отрисовкой персонажей. Во время разработки сервер сам обслуживает файлы из этого каталога — дополнительных настроек не требуется. Управление персонажем — клавишами `Q`, `W`, `E`, `A`, `S`, `D`. Фон поля чёрный, а каждая клетка визуализируется в стилистике присланного эталона.
