...
### Анимация персонажа из FR

В каталоге `client/player/` можно хранить оригинальные FR-файлы анимаций игрока. Клиент декодирует их прямо в браузере, применяя палитру Fallout (см. `client/fr/palette.js`), собирает `ImageBitmap` для каждого кадра и проигрывает последовательности в собственном рендер-цикле. По умолчанию ожидается файл `player/hero_run.fr`; если он недоступен или повреждён, клиент возвращается к упрощённому круговому маркеру.

Настройки анимации вынесены в секцию `[Player]` конфигурации UI и поддерживают следующие параметры:

- `RunAnimation` — путь до FR-файла с бегом (6 направлений по окружности). Файл должен лежать в статике клиента.
- `IdleAnimation` — опциональная анимация стоя. Если не задана, используются кадры бега с остановкой на первом кадре.
- `DirectionOrder` — соответствие индексов FR-направлений и гекс-направлений клиента (например, `southEast, east, northEast, northWest, west, southWest`).
- `MoveDuration` — длительность перехода между соседними гексами в миллисекундах (используется для интерполяции перемещения).
- `Scale` — коэффициент масштабирования спрайта относительно оригинального размера.
- `OffsetX` и `OffsetY` — дополнительные смещения от центра гекса после масштабирования (в пикселях), позволяющие точно совместить персонажа с сеткой.
- `DefaultFacing` — направление, которое используется до получения первых данных о перемещении (например, после спавна).

Клиент автоматически подгружает анимации при старте, синхронизирует кадры с частотой, указанной во FR, и воспроизводит бег при перемещении между гексами. Если какие-то файлы отсутствуют, в лог консоли выводится предупреждение, а отображение падает обратно на геометрический маркер.
